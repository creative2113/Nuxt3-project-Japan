...
<template>
  <div
    id="yoko-chin-band-outer-slider"
    class="mx-auto pt-10 pb-[30px] relative"
  >
    <Swiper
      ...
      :space-between="-30"
      :loop="true"
      class="!pb-10 !overflow-visible"
    >
      <SwiperSlide
        v-for="(band, index) in panels"
        :key="index"
        class="!w-[320px] rounded-[30px] overflow-hidden drop-shadow-xl"
      >
        <div
          id="yoko-chin-band-inner-slider"
          ...
          >
            <template v-for="(file, innerIndex) in band.files">
              <SwiperSlide v-if="file.file_path" :key="`innner-${innerIndex}`">
                <video
                  v-if="
                    (file && file.file_path?.endsWith('.mp4')) ||
                    file.file_path?.endsWith('.MP4') ||
                    file.file_path?.endsWith('.mov')
                  "
                  class="w-[320px] h-[520px] object-cover"
                  :src="file.file_path"
                  loop
                  muted
                  @mouseover="
                ...
                <div v-else class="relative">
                  ...
                  <div
                    class="relative z-10 w-full h-[530px] flex items-center justify-center"
                  >
                    <img
                      class="w-full"
                      :src="
        ...
        <div
          class="absolute z-10 flex pl-2.5 gap-1 w-full bottom-[130px] pb-2 left-0 border-b"
        >
          <div
            class="flex justify-center items-center pr-4 gap-1.5 rounded-full bg-white border border-[#DDDDDD]"
          >
            <img
              src="/images/dance-look-lesson-kids-1.webp"
              width="30"
              height="30"
              alt="offer"
              loading="lazy"
              ...
              class="w-[30px] h-auto"
            />
            <span class="text-xs font-bold text-center">未就学児年長</span>
          </div>
          <div
            class="flex justify-center items-center pr-4 gap-1.5 rounded-full bg-white border border-[#DDDDDD]"
          >
            <img
              src="/images/dance-look-lesson-kids-2.webp"
              width="30"
              height="30"
              alt="offer"
              loading="lazy"
              ...
              class="w-[30px] h-auto"
            />
            <span class="text-xs font-bold text-center">小学校低学年</span>
          </div>
        </div>
        <div class="relative z-10 bg-white w-full px-3 h-[150px]">
          ...
          <a
            :href="band.application_url"
            class="relative flex items-center justify-center text-xl text-white bg-[#000000] w-[300px] h-[50px] rounded-[30px] drop-shadow-md mt-2 after:content-[url('/images/arrow/arrow_right_white_link.svg')] after:absolute after:right-5 after:top-1/2 after:-translate-y-1/2"
            @click="$emit('application_click', band.eys_class_id)"
            >体験レッスン申し込み</a
          >
          <a
            :href="band.detail_url"
    ...
    <button
      class="yoko-chin-swiper-button-prev absolute top-1/2 -translate-y-1/2 !w-[50px] !h-[50px] bg-[#00000080] px-2.5 !left-0 z-10"
      aria-label="前へ"
    >
      <img
    ...
    <button
      class="yoko-chin-swiper-button-next absolute top-1/2 -translate-y-1/2 !w-[50px] !h-[50px] bg-[#00000080] px-2.5 !right-0 z-10"
      aria-label="次へ"
    >
      <img
...
<style>
#yoko-chin-band-outer-slider:not(#yoko-chin-band-inner-slider)
  .swiper-slide-active {
  z-index: 2;
}
