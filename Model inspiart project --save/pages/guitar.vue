...
<template>
  <LazySectionPageTop banner="/images/guitar-top-banner.webp" en_title="Guitar" title="ギターコースのご紹介"
    sub_title="アコースティックからエレキ<br>クラシックまで<br>全6ジャンルに対応！" bg-from="#77BBDD" bg-to="#6688DD" />
  ...
  <LazySectionPageTopAbout v-if="show0" :id="0" />
  <SectionStickyCTA v-if="show0" />

  <!-- First Section -->
  <div ref="section1">
    <LazyAtomPageSectionTitle :id="1" title="高品質なギターを<br>何と無料でプレゼント！" sub_title="楽器は買わずにもらっちゃおう！" color="#FC88A7" />
  </div>
  <LazySectionGuitarList v-if="show1" />
  <LazySectionFirstPageLayout v-if="show1">
    <SectionGuitarFirstPageContent />
  ...
  <SectionPageBeginner v-if="show2" :id="1" />
  ...
  <SectionInstructors v-if="show3" :id="1" />
  ...
  <!-- Fifth Section -->
  <div ref="section5">
    <LazyAtomPageSectionTitle :id="5" title="圧倒的に安い月会費、<br>通い方も自由自在" sub_title="アーティストの夢を実現するために安さは重要"
      color="#FF88BB" />
  </div>
  ...
  <SectionMakeDebut v-if="show6" title="ギターでプロデビューしよう！" banner="/images/make-debut-banner-1.webp" />
  <SectionProDebut v-if="show6" banner="/images/pro-debut-banner-1.webp" />
